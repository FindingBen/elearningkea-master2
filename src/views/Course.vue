<template>
  <main class="course-page">
    <section class="course-page__video">
      ...
      <bottomActions class="course-page__video-content-buttons" @previous="previous" @next="next" />
      <div class="course-page__video-description p-1" v-if="course">{{ course.courseDescription }}</div>
    </section>
    <section class="course-page__notes">
      ...
      <div class="course-page__notes-textarea">
        <textarea v-model="note.noteText"></textarea>
      </div>
    </section>
  </main>
</template>
...
export default {
  ...
  methods: {
    ...
    noteClicked(note) {
      this.activeVideo = this.course.video.find(
        _ => _.videoId === note.videoId
      );
      this.$nextTick(() => {
